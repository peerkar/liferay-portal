...
public class OpenSearchDocumentFactoryImplTest {
	...
	protected void assertDocumentLegacy(String value, String json) {
		...
		Assert.assertEquals(
			json, _openSearchDocumentFactory.getOpenSearchDocument(document));
	}
	...
	private void _assertDocument(
		...
		Assert.assertEquals(
			expectedJsonData.toString(), actualJsonData.toString());
	}
	...
	private void _assertDocument(
		...
		Assert.assertEquals(expected, jsonData.toString());
	}
