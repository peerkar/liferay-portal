...
public class IndexHelperImpl implements IndexHelper {
	...
	public void createIndex(
		...
		CreateIndexRequest.Builder createIndexRequestBuilder =
			_createCreateIndexRequestBuilder(
				JSONUtil.put(
					"mappings", mappingsFactory.getMappingsJSONObject()
				).put(
					"settings", _getSettingsJSONObject()
				));
		...
		JsonpUtil.logInfoResponse(
			...
			_getCreateIndexResponse(
				createIndexRequestBuilder.build(), openSearchIndicesClient));
		...
		if (PortalRunMode.isTestMode()) {
			...
			settingsFactory.setTestModeIndexSettings(openSearchIndicesClient);
		}
	}
	...
	public void deleteIndex(
		...
		try {
			...
			JsonpUtil.logInfoResponse(_log, deleteIndexResponse);
	...
	private JSONObject _dotNotationSettingsMapToJSON(
		...
		for (Map.Entry<String, String> entry : settingsMap.entrySet()) {
			...
			for (int i = settingParts.length - 1; i >= 0; i--) {
				if (i == (settingParts.length - 1)) {
					jsonObject.put(settingParts[i], entry.getValue());
				}
				else {
					jsonObject = JSONUtil.put(settingParts[i], jsonObject);
				}
			}
	...
	private JSONObject _executeIndexSettingsContributors(
		...
		Map<String, String> contributedSettingsMap = new HashMap<>();

		for (IndexSettingsContributor indexSettingsContributor :
				_indexSettingsContributorServiceTrackerList) {

			indexSettingsContributor.populate(contributedSettingsMap::put);
		}

		if (MapUtil.isEmpty(contributedSettingsMap)) {
		...
		try {
			return JSONUtil.merge(
				...
				_dotNotationSettingsMapToJSON(contributedSettingsMap));
		}
		catch (JSONException jsonException) {
			throw new RuntimeException(jsonException);
		}
