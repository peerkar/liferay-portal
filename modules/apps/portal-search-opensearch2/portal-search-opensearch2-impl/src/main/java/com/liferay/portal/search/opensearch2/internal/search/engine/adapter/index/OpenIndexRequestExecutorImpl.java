...
public class OpenIndexRequestExecutorImpl implements OpenIndexRequestExecutor {
	...
	protected OpenRequest createOpenRequest(OpenIndexRequest openIndexRequest) {
		OpenRequest.Builder openRequestBuilder = new OpenRequest.Builder();

		openRequestBuilder.index(
			ListUtil.fromArray(openIndexRequest.getIndexNames()));

		IndicesOptions indicesOptions = openIndexRequest.getIndicesOptions();

		if (indicesOptions != null) {
			openRequestBuilder.allowNoIndices(
				indicesOptions.isAllowNoIndices());
			openRequestBuilder.ignoreUnavailable(
