...
public class MultisearchSearchRequestExecutorImpl
	...
	private RequestItem _toRequestItem(SearchRequest searchRequest) {
		MultisearchBody.Builder multisearchBodyBuilder =
			new MultisearchBody.Builder();

		multisearchBodyBuilder.aggregations(searchRequest.aggregations());
		multisearchBodyBuilder.from(searchRequest.from());
		multisearchBodyBuilder.highlight(searchRequest.highlight());
		multisearchBodyBuilder.minScore(searchRequest.minScore());
		multisearchBodyBuilder.postFilter(searchRequest.postFilter());
		multisearchBodyBuilder.query(searchRequest.query());
		multisearchBodyBuilder.searchAfter(searchRequest.searchAfter());
		multisearchBodyBuilder.size(searchRequest.size());
		multisearchBodyBuilder.sort(searchRequest.sort());
		multisearchBodyBuilder.source(searchRequest.source());
		multisearchBodyBuilder.suggest(searchRequest.suggest());
		multisearchBodyBuilder.trackScores(searchRequest.trackScores());
		multisearchBodyBuilder.trackTotalHits(searchRequest.trackTotalHits());

		return RequestItem.of(
			requestItem -> requestItem.body(multisearchBodyBuilder.build()));
	}
