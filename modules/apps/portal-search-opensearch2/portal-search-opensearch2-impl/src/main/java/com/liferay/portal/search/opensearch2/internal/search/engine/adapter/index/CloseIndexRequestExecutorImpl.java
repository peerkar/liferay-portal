...
public class CloseIndexRequestExecutorImpl
	...
	protected org.opensearch.client.opensearch.indices.CloseIndexRequest
		...
		closeIndexRequestBuilder.index(
			ListUtil.fromArray(closeIndexRequest.getIndexNames()));

		IndicesOptions indicesOptions = closeIndexRequest.getIndicesOptions();

		if (indicesOptions != null) {
			closeIndexRequestBuilder.allowNoIndices(
				indicesOptions.isAllowNoIndices());
			closeIndexRequestBuilder.ignoreUnavailable(
		...
		if (closeIndexRequest.getTimeout() > 0) {
			...
			closeIndexRequestBuilder.masterTimeout(time);
			closeIndexRequestBuilder.timeout(time);
		}

		return closeIndexRequestBuilder.build();
	}
