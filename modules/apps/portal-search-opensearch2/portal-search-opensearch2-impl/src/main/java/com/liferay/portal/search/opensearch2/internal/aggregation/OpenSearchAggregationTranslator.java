...
public class OpenSearchAggregationTranslator
	...
	public org.opensearch.client.opensearch._types.aggregations.Aggregation
		...
		setRanges(dateRangeAggregation, dateRangeAggregationBuilder);
	...
	public org.opensearch.client.opensearch._types.aggregations.Aggregation
		...
		setRanges(rangeAggregation, rangeAggregationBuilder);
		SetterUtil.setNotNullScript(
	...
	protected void setRanges(
		...
		ranges.forEach(
			range -> dateRangeAggregationBuilder.ranges(
	...
	protected void setRanges(
		...
		org.opensearch.client.opensearch._types.aggregations.RangeAggregation.
			Builder rangeAggregationBuilder) {

		List<Range> ranges = rangeAggregation.getRanges();

		ranges.forEach(
			range -> rangeAggregationBuilder.ranges(
				_createAggregationRange(
	...
	private WeightedAverageValue _getWeightedAverageValue(
		...
		WeightedAverageValue.Builder weightedAverageValueBuilder =
			new WeightedAverageValue.Builder();

		weightedAverageValueBuilder.field(field);

		if (missing != null) {
			weightedAverageValueBuilder.missing(
				ConversionUtil.toDouble(missing));
		}

		if (script != null) {
			weightedAverageValueBuilder.script(
				scriptTranslator.translate(script));
		}

		return weightedAverageValueBuilder.build();
	}
