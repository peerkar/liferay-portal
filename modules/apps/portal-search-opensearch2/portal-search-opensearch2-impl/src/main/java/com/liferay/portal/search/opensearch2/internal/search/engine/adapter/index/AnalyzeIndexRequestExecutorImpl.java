...
public class AnalyzeIndexRequestExecutorImpl
	...
	protected AnalyzeRequest createAnalyzeRequest(
		...
		AnalyzeRequest.Builder analyzeRequestBuilder =
			new AnalyzeRequest.Builder();

		analyzeRequestBuilder.attributes(
			ListUtil.fromArray(analyzeIndexRequest.getAttributesArray()));
		analyzeRequestBuilder.explain(analyzeIndexRequest.isExplain());
		analyzeRequestBuilder.index(analyzeIndexRequest.getIndexName());
		analyzeRequestBuilder.text(
			ListUtil.fromArray(analyzeIndexRequest.getTexts()));

		if (Validator.isNotNull(analyzeIndexRequest.getAnalyzer())) {
			analyzeRequestBuilder.analyzer(analyzeIndexRequest.getAnalyzer());
		}
		else if (Validator.isNotNull(analyzeIndexRequest.getFieldName())) {
			analyzeRequestBuilder.field(analyzeIndexRequest.getFieldName());
		}
		else if (Validator.isNotNull(analyzeIndexRequest.getNormalizer())) {
			analyzeRequestBuilder.normalizer(
				analyzeIndexRequest.getNormalizer());
		}
		else {
			if (Validator.isNotNull(analyzeIndexRequest.getTokenizer())) {
				analyzeRequestBuilder.tokenizer(
					Tokenizer.of(
			...
			for (String charFilter : analyzeIndexRequest.getCharFilters()) {
				analyzeRequestBuilder.charFilter(
					CharFilter.of(
		...
		return analyzeRequestBuilder.build();
	}
