...
public class DeleteIndexRequestExecutorImpl
	...
	protected org.opensearch.client.opensearch.indices.DeleteIndexRequest
		...
		deleteIndexRequestBuilder.index(
			ListUtil.fromArray(deleteIndexRequest.getIndexNames()));

		IndicesOptions indicesOptions = deleteIndexRequest.getIndicesOptions();

		if (indicesOptions != null) {
			deleteIndexRequestBuilder.allowNoIndices(
				indicesOptions.isAllowNoIndices());
			deleteIndexRequestBuilder.ignoreUnavailable(
				indicesOptions.isIgnoreUnavailable());
		}

		return deleteIndexRequestBuilder.build();
	}
