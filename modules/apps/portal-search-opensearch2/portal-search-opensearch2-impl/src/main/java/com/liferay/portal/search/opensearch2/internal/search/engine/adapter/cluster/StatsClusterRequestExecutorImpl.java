...
public class StatsClusterRequestExecutorImpl
	...
	public StatsClusterResponse execute(
		...
		try {
			...
			ClusterNodes clusterNode = clusterStatsResponse.nodes();

			long availableSpace = clusterNode.fs(
			).availableInBytes();

			long totalSpace = clusterNode.fs(
			).totalInBytes();
	...
	private ClusterStatsRequest _createClusterStatsRequest(
		...
		ClusterStatsRequest.Builder clusterStatsRequestBuilder =
			new ClusterStatsRequest.Builder();

		if (ArrayUtil.isNotEmpty(statsClusterRequest.getNodeIds())) {
			clusterStatsRequestBuilder.nodeId(
				Arrays.asList(statsClusterRequest.getNodeIds()));
		}

		return clusterStatsRequestBuilder.build();
	}
