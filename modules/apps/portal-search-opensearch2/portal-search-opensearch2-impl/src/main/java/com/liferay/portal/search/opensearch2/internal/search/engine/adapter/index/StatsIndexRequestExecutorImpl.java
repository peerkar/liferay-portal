...
public class StatsIndexRequestExecutorImpl
	...
	private IndicesStatsRequest _createIndicesStatsRequest(
		...
		IndicesStatsRequest.Builder indicesStatsRequestBuilder =
			new IndicesStatsRequest.Builder();

		if (ArrayUtil.isNotEmpty(statsIndexRequest.getIndexNames())) {
			indicesStatsRequestBuilder.index(
				Arrays.asList(statsIndexRequest.getIndexNames()));
		}
		else {
			indicesStatsRequestBuilder.index("_all");
		}

		return indicesStatsRequestBuilder.build();
	}
