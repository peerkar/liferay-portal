...
public class BulkDocumentRequestExecutorImpl
	...
	public BulkDocumentResponse execute(
		...
		BulkResponse bulkResponse = _getBulkResponse(
			createBulkRequest(bulkDocumentRequest), bulkDocumentRequest);
		...
		for (BulkResponseItem bulkResponseItem : bulkResponse.items()) {
			...
			if (errorCause != null) {
				...
				// TODO: Not present in the Java API

				//bulkDocumentItemResponse.setAborted(true);
				...
				request -> {
					if (request instanceof DeleteDocumentRequest) {
						...
						bulkRequestBuilder.operations(
							new BulkOperation(deleteOperation));
					}
						...
						bulkRequestBuilder.operations(
							new BulkOperation(indexOperation));
					}
						...
						bulkRequestBuilder.operations(
							new BulkOperation(updateOperation));
					}
					else {
