...
public class CommonSearchRequestBuilderAssemblerImpl
	...
	private org.opensearch.client.opensearch._types.query_dsl.Query
		...
		if (!postFilterQueryParts.isEmpty()) {
			query = _combine(query, postFilterQueryParts);
		}
	...
	private org.opensearch.client.opensearch._types.query_dsl.Query _combine(
		...
		if (query != null) {
			biConsumer.accept(boolQueryBuilder, query);
		}
	...
	private BoolQuery.Builder _createBoolQueryBuilder(BoolQuery boolQuery) {
		...
		boolQueryBuilder.filter(boolQuery.filter());
		boolQueryBuilder.minimumShouldMatch(boolQuery.minimumShouldMatch());
		boolQueryBuilder.must(boolQuery.must());
		boolQueryBuilder.mustNot(boolQuery.mustNot());
		boolQueryBuilder.queryName(boolQuery.queryName());
	...
	private org.opensearch.client.opensearch._types.query_dsl.Query _getQuery(
		...
		return _combine(query1, complexQueryParts);
	}
	...
	private void _setIndexBoosts(
		...
		Map<String, Double> indicesBoost = new HashMap<>();

		MapUtil.isNotEmptyForEach(
			baseSearchRequest.getIndexBoosts(),
			(indexName, boost) -> indicesBoost.put(
				indexName, boost.doubleValue()));

		searchRequestBuilder.indicesBoost(indicesBoost);
	}
	...
	private void _setPointInTime(
		...
		pitBuilder.id(pointInTime.getPointInTimeId());

		if (pointInTime.getKeepAlive() != 0) {
			pitBuilder.keepAlive(
				pointInTime.getKeepAlive() + TimeUnit.Seconds.jsonValue());
		}

		searchRequestBuilder.pit(pitBuilder.build());
	}
