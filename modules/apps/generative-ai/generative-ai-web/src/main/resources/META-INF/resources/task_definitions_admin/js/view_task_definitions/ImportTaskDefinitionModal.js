...
const ImportTaskDefinitionModal = ({portletNamespace, redirectURL}) => {
	...
	const _handleSubmit = async () => {
		...
		try {
			const fetchURL = '/o/generative-ai/v1.0/task-definitions';
					...
					if (!ok) {
						if (
							responseContent.type.includes(
								'DuplicateTaskDefinitionExternalReferenceCodeException'
							)
						) {
							_handleFormError(Liferay.Language.get(
											'unable-to-import-task-definition-with-the-same-external-reference-code-as-an-existing-task-definition'
									  )
							);
						}
						else {
							_handleFormError(Liferay.Language.get(
											'unable-to-import-because-the-task-definition-configuration-is-invalid'
									  )
							);
						}
	...
	return visible ? (
		<ClayModal
			className="generative-ai-import-modal-root"
			observer={observer}
			size="full-screen"
		>
			<ClayModal.Header>
			...
			<ClayModal.Body>
				...
				<p className="text-secondary">
					{Liferay.Language.get(
						'select-a-task-definition-to-import'
					)}
				</p>
...
export default ImportTaskDefinitionModal;
