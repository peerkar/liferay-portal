...
public class ExportImportSiteProviderImpl implements ExportImportSiteProvider {
	...
	public List<Group> getSupportedSites(
		...
		return ListUtil.subList(
			_getSupportedSites(companyId, search, comparator), start, end);
	}
	...
	public int getSupportedSitesCount(long companyId, String search)
		...
		List<Group> groups = _getSupportedSites(companyId, search, null);
	...
	private List<Group> _getSupportedSites(
		...
		// Ordered here rather than by the query, because the name a site goes
		// by is not the name it is stored under and only the former is worth
		// ordering on
