...
/**
 * Liferay GSearch Decay function score query translator impl.
 * 
...
@Component(
	immediate=true,
	service = GSearchDecayFunctionScoreQueryTranslator.class
)
public class GSearchDecayFunctionScoreQueryTranslatorImpl
	extends BaseGSearchFunctionScoreQueryTranslator
	implements GSearchDecayFunctionScoreQueryTranslator {

	public QueryBuilder translate(GSearchDecayFunctionScoreQuery query) {
		
		DecayFunctionBuilder functionBuilder;

		if (query.getFunctionType().equals("exp")) {

			functionBuilder = new ExponentialDecayFunctionBuilder(
					query.getFieldName(), query.getOrigin(),
					query.getScale(), query.getOffset(), query.getDecay());
		} else if (query.getFunctionType().equals("linear")) {

			functionBuilder = new LinearDecayFunctionBuilder(
					query.getFieldName(), query.getOrigin(),
					query.getScale(), query.getOffset(), query.getDecay());
		} else {

			functionBuilder = new GaussDecayFunctionBuilder(
					query.getFieldName(), query.getOrigin(),
					query.getScale(), query.getOffset(), query.getDecay());
		}
