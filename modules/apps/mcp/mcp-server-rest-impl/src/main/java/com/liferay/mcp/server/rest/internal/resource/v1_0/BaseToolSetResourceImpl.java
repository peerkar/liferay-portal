...
public abstract class BaseToolSetResourceImpl implements ToolSetResource {
	...
	@io.swagger.v3.oas.annotations.Operation(
		description = "Returns every tool set exposed by Liferay, each with a curated description. Start here when `getToolSearchPage` is not among the tools available to you. When it is, search first and come here only after it has returned nothing relevant for the user's request; browsing tool sets costs far more context than a search. Pick the tool set whose description matches the user's intent and pass its `name` to `getToolSetToolSetNameToolSummariesPage` to see what tools it offers."
	)
...
// LIFERAY-REST-BUILDER-HASH:1663481643